...
def run(init_lr=0.1,
    ...
    for data in dataloaders["test"]:
        ...
        # per-class accuracy
    top1_div=top1_tp / (top1_tp + top1_fp)
    top1_div[np.isnan(top1_div)] = 0
    top1_per_class = np.mean(top1_div)
    top5_div=top5_tp / (top5_tp + top5_fp)
    top5_div[np.isnan(top5_div)] = 0
    top5_per_class = np.mean(top5_div)
    top10_div=top10_tp / (top10_tp + top10_fp)
    top10_div[np.isnan(top10_div)] = 0
    top10_per_class = np.mean(top10_div)
